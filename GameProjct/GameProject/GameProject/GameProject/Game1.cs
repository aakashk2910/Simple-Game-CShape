    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < bears.Count - 1 ; i++)
            {
                ...
                {
                    if (bears[j].Active && bears[i].Active)
                    {
            ...
            foreach (TeddyBear bear in bears)
            {
                    ...
                    bear.CollisionRectangle.Intersects(burger.CollisionRectangle))
                {
                    ...
                    burgerDamage.Play(0.5f, 0.5f, 0f);

                    burger.Health -= GameConstants.BEAR_DAMAGE;
                    healthString = GameConstants.HEALTH_PREFIX + burger.Health;
                    CheckBurgerKill();      //checking burger's life
        ...
        private void SpawnBear()
        {
            ...
            float v = GameConstants.MIN_BEAR_SPEED + RandomNumberGenerator.NextFloat(RandomNumberGenerator.NextFloat((GameConstants.BEAR_SPEED_RANGE)));
            float vAngle = (float)Math.PI * RandomNumberGenerator.NextFloat(2);
            
            Vector2 bearspeed = new Vector2(v * (float)Math.Cos(vAngle), v * (float)Math.Sin(vAngle));

            // create new bear

            TeddyBear newBear = new TeddyBear(Content, "teddybear", x, y, bearspeed, teddyBounce, teddyShot);

            // make sure we don't spawn into a collision

            List<Rectangle> collisionRectangle = new List<Rectangle>();
